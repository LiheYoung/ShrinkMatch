...
class WideResNetVar(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, is_remix=False, need_head_aux=False):
        ...
        if need_head_aux:
            self.head_aux = nn.Sequential(
                nn.Linear(channels[4], channels[4] // 4),
                nn.BatchNorm2d(channels[4] // 4, momentum=0.001, eps=0.001),
                nn.LeakyReLU(negative_slope=0.1, inplace=False),
                
                nn.Linear(channels[4] // 4, channels[4] // 4),
                nn.BatchNorm2d(channels[4] // 4, momentum=0.001, eps=0.001),
                nn.LeakyReLU(negative_slope=0.1, inplace=False),
    ...
    def forward(self, x, need_head_aux=False):
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
...
class build_WideResNetVar:
    def __init__(self, first_stride=1, depth=28, widen_factor=2, bn_momentum=0.01, leaky_slope=0.0, dropRate=0.0,
                 use_embed=False, is_remix=False, need_head_aux=False):
        ...
        self.is_remix = is_remix
        self.need_head_aux = need_head_aux
    ...
    def build(self, num_classes):
        return WideResNetVar(
            ...
            need_head_aux=self.need_head_aux
        )
