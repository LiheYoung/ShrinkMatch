...
from .shrinkmatch_utils import consistency_loss_with_shrink, Get_Scalar
...
class ShrinkMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u, \
        ...
        """
        class ShrinkMatch contains setter of data_loader, optimizer, and model update methods.
        Args:
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'], self.loader_dict['train_ulb']):
            ...
            with amp_cm():
                logits, logits_aux = self.model(inputs, need_head_aux=True)
                logits_x_lb = logits[:num_lb]
                logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                logits_aux_x_ulb_s = logits_aux[num_lb:].chunk(2)[1]
                ...
                certain_loss, mask, shrink_loss, removed_class_idx, uncertainty_avg, p_model = \
                    consistency_loss_with_shrink(logits_x_ulb_s, logits_x_ulb_w, logits_aux_x_ulb_s, p_cutoff, args.use_da, p_model)
                
                certain_ratio_ema = mask.item() if certain_ratio_ema is None else (certain_ratio_ema * ema_decay + mask.item() * (1 - ema_decay))

                total_loss = sup_loss + self.lambda_u * (certain_loss + shrink_loss * certain_ratio_ema) # for our loss reweighting principle 2
            ...
            tb_dict['train/certain_loss'] = certain_loss.detach()
            tb_dict['train/shrink_loss'] = shrink_loss.detach() if type(shrink_loss) == torch.Tensor else shrink_loss
            tb_dict['train/shrink_loss_reweighted'] = (shrink_loss.detach() * certain_ratio_ema) if type(shrink_loss) == torch.Tensor else shrink_loss
            tb_dict['train/removed_class_idx'] = removed_class_idx
            tb_dict['train/uncertainty_avg'] = uncertainty_avg
            tb_dict['train/certain_ratio_ema'] = certain_ratio_ema
            tb_dict['train/total_loss'] = total_loss.detach()
            tb_dict['train/mask_ratio'] = mask.detach()
