...
def net_builder(net_name, from_name: bool, net_conf=None, is_remix=False, need_head_aux=False):
    """
    ...
    if from_name:
        ...
        if net_name == 'WideResNet':
            ...
            builder = getattr(net, 'build_WideResNet')(need_head_aux=need_head_aux)
        elif net_name == 'WideResNetVar':
            import models.nets.wrn_var as net
            builder = getattr(net, 'build_WideResNetVar')(need_head_aux=need_head_aux)
        elif net_name == 'ResNet50':
            import models.nets.resnet50 as net
            builder = getattr(net, 'build_ResNet50')(is_remix, need_head_aux=need_head_aux)
        else:
...
def get_logger(name, save_path=None, level='INFO'):
    ...
    if not save_path is None:
        ...
        fileHandler = logging.FileHandler(os.path.join(save_path, str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S')) + '.log'))
