...
from models.shrinkmatch.shrinkmatch import ShrinkMatch
from datasets.ssl_dataset import SSL_Dataset, ImageNetLoader, SemiAves
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'imagenet' in args.dataset.lower():
        _net_builder = net_builder('ResNet50', False, None, is_remix=False, need_head_aux=True)
    else:
    ...
    ## set SGD and cosine lr on ShrinkMatch
    ...
    elif args.dataset == 'aves':
        lb_dset = SemiAves(root='./data/semi_aves', split='labeled')
        ulb_dset = SemiAves(root='./data/semi_aves', split='unlabeled')
        eval_dset = SemiAves(root='./data/semi_aves', split='test')
    else:
        train_dset = SSL_Dataset(args, alg='shrinkmatch', name=args.dataset, train=True,
                                num_classes=args.num_classes, data_dir=args.data_dir)
        lb_dset, ulb_dset = train_dset.get_ssl_dset(args.num_labels)

        _eval_dset = SSL_Dataset(args, alg='shrinkmatch', name=args.dataset, train=False,
                                num_classes=args.num_classes, data_dir=args.data_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str, default='shrinkmatch')
