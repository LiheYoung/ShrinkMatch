...
class SemiAves(Dataset):
    def __init__(self, root, split):
        super(SemiAves, self).__init__()
        self.root = root
        self.split = split
        with open(os.path.join('partitions/semi_aves', split + '.txt'), 'r') as f:
    ...
    def __getitem__(self, idx):
        imgname, target = self.filenames[idx].split(' ')
        img = dataset_parser.default_loader(os.path.join(self.root, imgname))
        
        img_w = self.transform(img)
        
        if self.split == 'unlabeled':
            img_s = self.transform(img)
            return imgname, img_w, img_s
        else:
            return imgname, img_w, int(target)
